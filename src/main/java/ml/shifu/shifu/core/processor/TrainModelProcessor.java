...
import ml.shifu.shifu.core.alg.DNNTrainer;
import ml.shifu.shifu.core.alg.DNNTrainerTF;
...
public class TrainModelProcessor extends BasicModelProcessor implements Processor {
    ...
    public int run() throws Exception {
        ...
        try {
            ...
            //LOG.info(runMode.name());
            switch(runMode) {
                case DIST:
                case MAPRED:
                	if(!modelConfig.getAlgorithm().equalsIgnoreCase("DNN")) {
                    	...
                    	//runTFTrain();
                    	runTFTrain();
                    	String dataFileDir = pathFinder.getNormalizedDataPath().substring(FileSystem.get(new Configuration()).
                    			getHomeDirectory().toString().length());
                    	new Client().run(pathFinder.getNormalizedDataPath()+"/","0.11.1");
                    }
                    break;
                case LOCAL:
                	//if(this.modelConfig.)
    ...
    /**
     * run training process using KERAS
     * 
     * @param 
     *            number of bags, it decide how much trainer will start training
     * @throws Exception 
    ...
    private void runKerasTrain(){
		...
		e.printStackTrace(System.out);
        	throw new RuntimeException("Train or Build Model Failure:\n" + e.getMessage());
        }
    }
    private void runTFTrain(){
    	String select_status = "";
        int count = 0;
    	for(ColumnConfig cc:this.columnConfigList) {
    		if(cc.isTarget()) {
    ...
    private void validateDistributedTrain() throws IOException {
        ...
        || CommonUtils.isTreeModel(alg)) && !alg.equalsIgnoreCase("DNN")) { // RF or GBT algortihm
        ...
        if(modelConfig.isClassification() && modelConfig.getTrain().isOneVsAll() && !CommonUtils.isTreeModel(alg)
                && !NNConstants.NN_ALG_NAME.equalsIgnoreCase(alg) && !alg.equalsIgnoreCase("DNN")) {
        ...
        if(!LogisticRegressionContants.LR_ALG_NAME.equalsIgnoreCase(alg)
                ...
                && gridSearch.hasHyperParam() && !alg.equalsIgnoreCase("DNN")) {
